function [cost_J] = simulate_irrigation_ndvi(Q_cost,R_cost,set_point,low_limit,high_limit,previous_eto,x_0,ndvi_initial,ndvi_limit,ndvi_irr,horizon,verbose) 
...
for i=1:1:horizon*samplings_per_day
    ...
    if mod(i,1440)==0
        if current_ndvi<ndvi_limit
            ir=on_irrigation;
        end    
    end
    if mod(i,1440)==ndvi_irr
        ir=off_irrigation;
    end
    % Calculate cost
...
if verbose
    ...
    display('=================================');
    fprintf('NDVI Irrigation Cost: %5.2f\n',cost_J);
    display('=================================');
end
